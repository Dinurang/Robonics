  ...
  return (
    <>
      ...
      <div className="dashboard-container">
        ...
        <nav className="dashboard-nav">
          <div className="nav-container">
            <Link to="/" className="logo">
              ...
              <span className="dashboard-brand">Admin Dashboard</span>
            </Link>
            ...
            <div className="nav-links">
              ...
              {/* View Orders */}
              <Link 
                to="/admin/dashboard/vieworders" 
                className={`nav-link ${activePage === 'vieworders' ? 'active' : ''}`}
              >
              ...
              {/* View Payments */}
              <Link 
                to="/admin/dashboard/viewpayments" 
                className={`nav-link ${activePage === 'viewpayments' ? 'active' : ''}`}
              >
              ...
              {/* Update Orders */}
              <Link 
                to="/admin/dashboard/updateorders" 
                className={`nav-link ${activePage === 'updateorders' ? 'active' : ''}`}
              >
              ...
              {/* Update Pricing */}
              <Link 
                to="/admin/dashboard/updatepricing" 
                className={`nav-link ${activePage === 'updatepricing' ? 'active' : ''}`}
              >
                <i className="fas fa-dollar-sign me-2"></i>
                <span>Update Pricing</span>
              </Link>

              {/* Manage Staff - Only show for owner role */}
              {(user?.role?.toLowerCase() === 'owner') && (
                <Link 
                  to="/admin/dashboard/managestaff" 
                  className={`nav-link ${activePage === 'managestaff' ? 'active' : ''}`}
                >
              ...
              {/* Show locked version for administrators */}
              {(user?.role?.toLowerCase() === 'administrator' || user?.role?.toLowerCase() === 'admin') && (
                <div className="nav-link disabled" style={{ opacity: 0.5, cursor: 'not-allowed' }}>
                  <i className="fas fa-users-cog me-2"></i>
                  <i className="fas fa-lock me-1" style={{ fontSize: '0.8rem' }}></i>
                  <span>Manage Staff</span>
              ...
              <div className="dropdown-custom-container" style={{ position: 'relative' }}>
                ...
                >
                  ...
                  {user?.role?.toLowerCase() === 'owner' && (
                    <span className="ms-2 badge bg-warning" style={{ fontSize: '0.7rem' }}>
                      Owner
                    </span>
                  )}
                  ...
                  <div className="dropdown-custom-menu">
                    <div className="dropdown-custom-header">
                      ...
                      <div className="mt-1">
                        ...
                        <span className={`badge ${user?.role?.toLowerCase() === 'owner' ? 'bg-warning' : 'bg-purple'}`}>
                          {user?.role}
                        </span>
                      </div>
                    </div>
        ...
        <header className="dashboard-header">
          <div className="header-content">
            <h1>Welcome back, <span className="highlight">{user?.username?.split('@')[0] || 'Admin'}</span>!</h1>
            <p>Manage Robonics {user?.role?.toLowerCase() === 'owner' && <span className="badge bg-warning ms-2">Owner</span>}</p>
          </div>
        </header>
        ...
        <footer className="dashboard-footer">
          <div className="footer-content">
            ...
            <div className="footer-links">
              <span>Logged in as: <strong>{user?.username}</strong> | Role: <strong>
                <span className={user?.role?.toLowerCase() === 'owner' ? 'text-warning' : 'text-info'}>
                  {user?.role}
                </span>
              </strong></span>
            </div>
      ...
      <style jsx>{`
        ...
        .nav-link:hover:not(.btn-myaccount):not(.disabled) {
